...
function renderDeck(data) {
    ...
    if (titleEl)
        titleEl.textContent = data.name || 'Untitled Deck';
    if (strategyEl)
        strategyEl.innerHTML = `<p>${data.strategyDescription || ''}</p>`;
    if (cardsEl) {
        cardsEl.innerHTML = '';
...
async function init() {
    ...
    try {
        ...
        if (err && err.name === 'DeckValidationError') {
